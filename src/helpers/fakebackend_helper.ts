...
export const postJwtRegister = (url : string, data  :any) => {
  const mappedData = {
    ...
    nombre_completo: data.first_name || data.nombre_completo,
    rol_global: data.rol_global || "Miembro"
  };
  return api.create(url, mappedData)
    .catch(err => {
      ...
      if (err.response && err.response.status) {
        switch (err.response.status) {
          ...
          case 401:
            ...
            break;
          default:
            message = err.message || (Array.isArray(err) ? err[1] : err);
            break;
        }
      }
      throw message || err;
