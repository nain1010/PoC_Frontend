...
const Analytics = () => {
    ...
    // Data states
    const [velocityData, setVelocityData] = useState<{ sprints: SprintVelocity[], velocidad_promedio: number }>({ sprints: [], velocidad_promedio: 0 });
    ...
    const fetchAnalyticsData = async () => {
        ...
        setError(null);
        try {
            // Fetch velocity & capacity in parallel
            const [velocityRes, capacityRes] = await Promise.all([
                api.get(`/projects/${activeProjectId}/velocity`),
                api.get(`/projects/${activeProjectId}/capacity`)
            ]);

            setVelocityData(velocityRes as any || { sprints: [], velocidad_promedio: 0 });
            setCapacityData(capacityRes || null);
        } catch (err: any) {
            setError(err || "Error al cargar los datos analíticos.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const storiesTotal = sprintResumen?.historias_totales || 0;
    const storiesDone = sprintResumen?.historias_completadas || 0;
    const pointsTotal = sprintResumen?.puntos_totales || 0;
    const pointsDone = sprintResumen?.puntos_completados || 0;

    const activeSprintStoryPercent = storiesTotal > 0 ? Math.round((storiesDone / storiesTotal) * 100) : 0;
    const activeSprintPointsPercent = pointsTotal > 0 ? Math.round((pointsDone / pointsTotal) * 100) : 0;
                                        ...
                                        <CardBody className="p-3">
                                            {!activeSprintInfo ? (
                                                ...
                                                <div className="text-center py-5 text-muted fs-13">
                                                    <i className="ri-checkbox-circle-line text-success align-middle me-1"></i> ¡Excelente! Todas las tareas de este sprint están asignadas.
                                                </div>
                                            ) : (
                                                                ...
                                                                <tr key={task.id}>
                                                                    ...
                                                                    <td>
                                                                        <span className={`badge bg-soft-${task.estado === 'Pendiente' ? 'secondary' :
                                                                            task.estado === 'En Curso' ? 'primary' :
                                                                                task.estado === 'Bloqueada' ? 'danger' : 'success'
                                                                            } fs-11`}>
                                                                            {task.estado}
                                                                        </span>
                                                                    </td>
...
const VelocitySvgChart = ({ data }: { data: SprintVelocity[] }) => {
    // Canvas dimensions
    const width = 500;
    const height = 250;
...
const BurndownSvgChart = ({ sprintInfo, resumen }: { sprintInfo: any, resumen: any }) => {
    // Canvas dimensions
    const width = 500;
    const height = 250;
    const paddingLeft = 40;
