...
const KanbanStoryCard = React.memo(({ story, projectDetails, memberFilter, onStoryStatusChange, onTaskStatusChange, onTaskAssign, onOpenTaskModal, expanded, onToggleExpand, onOpenPageSelector, onOpenAttachmentModal, onOpenPageViewer, dragHandleProps, setNodeRef, style, isDragging, statusErrorExt }: {
    ...
    const parseCriterios = useCallback((criterios: any): string[] => {
        if (!criterios) return [];
    ...
    const criteriosList = useMemo(() => parseCriterios(story.criterios_aceptacion), [story.criterios_aceptacion, parseCriterios]);
    ...
    return (
        ...
        >
            <div className="card-body">
                ...
                {/* Actions — Details toggle + Tasks toggle + Create Task */}
                <div className="kanban-card-actions">
                    <button 
                        type="button"
                        className={`kanban-card-actions__toggle ${showDetails ? 'active text-primary' : ''}`}
                        onClick={handleToggleDetails}
                        title="Ver narrativa completa y criterios de aceptación"
                    >
                        <i className={showDetails ? 'ri-file-text-fill' : 'ri-file-text-line'}></i>
                        <span>{showDetails ? 'Ocultar Criterios' : 'Criterios'}</span>
                    </button>
                    ...
                    >
                        ...
                        <span>{expanded ? 'Ocultar Tareas' : `Tareas (${displayedTasks.length})`}</span>
                    </button>
